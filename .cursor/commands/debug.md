...
- MCP logs: `~/.bot-bot/logs/mcp-claude-approvals-*.log`
- Combined WUI/Daemon logs: `~/.bot-bot/logs/wui-${BRANCH_NAME}/codelayer.log`
- First line shows: `[timestamp] starting [service] in [directory]`

**Database**:
- Location: `~/.bot-bot/daemon-{BRANCH_NAME}.db`
...
**Service Status**:
- Check if daemon is running: `ps aux | grep agent-daemon`
- Check if WUI is running: `ps aux | grep wui`
- Socket exists: `~/.bot-bot/daemon.sock`
...
1. Find latest daemon log: ls -t ~/.bot-bot/logs/daemon-*.log | head -1
2. Find latest WUI log: ls -t ~/.bot-bot/logs/wui-*.log | head -1
...
1. Connect to database: sqlite3 ~/.bot-bot/daemon.db
...
2. **If That Doesn't Work**:
   ...
   - Run with debug: `BOT_BOT_DEBUG=true make daemon`
...
**Find Latest Logs**:
```bash
ls -t ~/.bot-bot/logs/daemon-*.log | head -1
ls -t ~/.bot-bot/logs/wui-*.log | head -1
```

**Database Queries**:
```bash
sqlite3 ~/.bot-bot/daemon.db ".tables"
sqlite3 ~/.bot-bot/daemon.db ".schema sessions"
sqlite3 ~/.bot-bot/daemon.db "SELECT * FROM sessions ORDER BY created_at DESC LIMIT 5;"
```

**Service Check**:
```bash
ps aux | grep agent-daemon     # Is daemon running?
